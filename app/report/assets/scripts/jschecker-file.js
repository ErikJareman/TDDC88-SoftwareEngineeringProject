...
$(function() {
  ...
  // bootstrap popover
	...
	function focusFragment() {
		$('.jschecker-mark').removeClass('focus');
	...
	var options = {
		lineNumbers: true,
		gutters: ['jschecker-gutter-jshint', 'jschecker-gutter-complexity'],
		readOnly: 'nocursor'
	};
	...
	var gutterIcon = $('<a><i class="jschecker-gutter-icon icon-cog"></i></a>');
	...
	var popovers = cm.operation(function() {
		...
		__report.complexity.functions.forEach(function(fn, i) {
			...
			var line = fn.line - 1;
			var className = 'jschecker-mark-fn-' + i;
			var gutter = {
				gutterId: 'jschecker-gutter-complexity',
	...
	var scrollToLine = function(i) {
		...
		window.location.hash = '#jschecker-mark-fn-' + i;
		...
		report.eslint.messages.forEach(function(message) {
			...
			if (_.isArray(message.line)) {
				message.line.forEach(function(line) {
					...
					lines[line] += '<div class="jschecker-jshint-message text-' + message.severity + '">' + text + '</div>';
				});
			} else {
				if (!lines[message.line]) lines[message.line] = '';
				lines[message.line] += '<div class="jschecker-jshint-message text-' + message.severity + '">' + text + '</div>';
			}
		});
		var marker = document.createElement('a');
		marker.innerHTML = '<i class="jschecker-gutter-icon icon-eye-open"></i>';
		Object.keys(lines).forEach(function(line) {
			var lineWidget = document.createElement('div');
			lineWidget.innerHTML = lines[line];
			cm.setGutterMarker(line - 1, 'jschecker-gutter-jshint', marker.cloneNode(true));
