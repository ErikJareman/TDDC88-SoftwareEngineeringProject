...
(function() {
	...
	CodeMirror.prototype.markPopoverText = function(lineObj, regex, className, gutter, message) {
		...
		var match, internalClass = 'jschecker-mark-' + makeid(10);
		while (match = cursor.findNext()) {
			...
			this.markText({
				...
				className: 'jschecker-mark ' + internalClass + ' ' + (className || ''),
				startStyle: 'jschecker-mark-start',
				endStyle: 'jschecker-mark-end'
			});
		}
    ...
    // the codemirror operations are executed
		return function() {
			var markStart = $('.jschecker-mark-start.' + internalClass);
